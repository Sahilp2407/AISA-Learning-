...
export async function submitExamAttempt(payload) {
  ...
  try {
    ...
    console.log('✅ Exam attempt recorded in Cloud Firestore with ID:', docRef.id);
    ...
    console.warn('⚠️ Cloud Firestore write for exam attempt failed, using persistent local store:', err);
 ...
 */
export async function fetchStudentExamAttempts(studentEmail = 'aditi.sharma@univ.edu') {
  const cloudAttempts = [];
...
export async function fetchAllExamAttempts() {
  ...
  const cloudIds = new Set(all.map(a => a.id));
